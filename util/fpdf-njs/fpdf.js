var lib = {
	str_replace : function(str, src, dst) {
		return str.replace(src, dst)
	},
	strlen : function(arr) {
		return arr.length;
	},
	strpos :  function(str, char){
		return str.indexOf(char);
	},
	substr : function(str, pos, size){
		return str.substring(pos, size)
	},
...
function FPDF(xres) {
	...
	this.SetLeftMargin = function SetLeftMargin(xmargin) {
		this.lMargin = xmargin;
		if (this.page > 0 && this.x < xmargin) this.x = xmargin;
	}
	this.SetTopMargin = function SetTopMargin(xmargin) {
		this.tMargin = xmargin;
	}
	this.SetRightMargin = function SetRightMargin(xmargin) {
		this.rMargin = xmargin;
	}
	...
	this.SetCompression = function SetCompression(xcompress) {
		...
		if (this.function_exists("gzcompress")) this.compress = xcompress;
		else
			this.compress = false;
	}
	...
	this.SetTitle = function SetTitle(xtitle) {
		this.title = xtitle;
	}
	this.SetSubject = function SetSubject(xsubject) {
		this.subject = xsubject;
	}
	this.SetAuthor = function SetAuthor(xauthor) {
		this.author = xauthor;
	}
	this.SetKeywords = function SetKeywords(xkeywords) {
		this.keywords = xkeywords;
	}
	this.SetCreator = function SetCreator(xcreator) {
		this.creator = xcreator;
	}
	this.Error = function Error(xmsg) {
		//Response.Write("<B>FPDF error: </B>" + xmsg);
		console.log("<B>FPDF error: </B>" + xmsg);
		//Response.End
	}
	...
	this.AddPage = function AddPage(xorientation) {
		...
		this._out("2 J");
		this.LineWidth = xlw;
		this._out(this.sprintf("%.2f w", xlw * this.k));
	...
	this.SetDrawColor = function SetDrawColor(xr, xg, xb) {
		...
		if ((xr == 0 && xg == 0 && xb == 0) || xg == -1) this.DrawColor = this.sprintf("%.3f G", xr / 255);
		else this.DrawColor = this.sprintf("%.3f %.3f %.3f RG", xr / 255, xg / 255, xb / 255);
	...
	this.SetFillColor = function SetFillColor(xr, xg, xb) {
		...
		if ((xr == 0 && xg == 0 && xb == 0) || xg == -1) this.FillColor = this.sprintf("%.3f g", xr / 255);
		else {
			this.FillColor = this.sprintf("%.3f %.3f %.3f rg", xr / 255, xg / 255, xb / 255)
		};
	...
	this.SetTextColor = function SetTextColor(xr, xg, xb) {
		...
		if ((xr == 0 && xg == 0 && xb == 0) || xg == -1) this.TextColor = this.sprintf("%.3f g", xr / 255);
		else this.TextColor = this.sprintf("%.3f %.3f %.3f rg", xr / 255, xg / 255, xb / 255);
		this.ColorFlag = (this.FillColor != this.TextColor);
	}
	...
	this.GetStringWidth = function GetStringWidth(xs) {
		...
		for (xi = 0; xi < xl; xi++) {
			var index = xcw.indexOf(xs.charAt(xi))
			xw = xw + (xcw[index+1]);
		}
	...
	this.SetLineWidth = function SetLineWidth(xwidth) {
		...
		if (this.page > 0) this._out(this.sprintf("%.2f w", xwidth * this.k));
	}
	...
	this.Line = function Line(xx1, xy1, xx2, xy2) {
		this._out(this.sprintf("%.2f %.2f m %.2f %.2f l S", xx1 * this.k, (this.h - xy1) * this.k, xx2 * this.k, (this.h - xy2) * this.k));
	}
	...
	this.Rect = function Rect(xx, xy, xw, xh, xstyle) {
		...
		if (xstyle == "F") xop = "f";
			...
			xop = "S";
		this._out(this.sprintf("%.2f %.2f %.2f %.2f re %s", xx * this.k, (this.h - xy) * this.k, xw * this.k, -xh * this.k, xop));
	}
	...
	this.SetFont = function SetFont(xfamily, xstyle, xsize) {
		...
		if (xfontkey === 'courier') {
			xfpdf_charwidths["courier"] = []
			for (var i = 0; i <= 255; i++) xfpdf_charwidths["courier"][String.fromCharCode(i)] = 600;
		...
		// If we don't know the font's widths, see if we have them on file..
		if (!this.fonts[xfontkey]) {
			if (this.CoreFonts[xfontkey]) {
				if (!xfpdf_charwidths[xfontkey]) {
					...
					xfpdf_charwidths[xfontkey] = require(xfile);

					if (xfpdf_charwidths[xfontkey] === undefined) this.Error('Could not include font metric file "' + xfontkey + '"');
				}
				//xi=lib.count(this.fonts)+1;
				xi = this.fonts.length + 1;
				//this.fonts[xfontkey] = ["i", xi, "type", "core", "name", this.CoreFonts[xfontkey], "up", -100, "ut", 50, "cw", xfpdf_charwidths[xfontkey]];
				this.fonts[xfontkey] = {"i":xi, "type":"core", "name":this.CoreFonts[xfontkey], "up":-100, "ut":50, "cw":xfpdf_charwidths[xfontkey]};
			} else
		...
		if (this.page > 0) this._out(this.sprintf("BT /F%d %.2f Tf ET", this.CurrentFont["i"], this.FontSizePt));
	}
	...
	this.Text = function Text(xx, xy, xtxt) {
		...
		xs = this.sprintf("BT %.2f %.2f Td (%s) Tj ET", xx * this.k, (this.h - xy) * this.k, xtxt);
	...
	this.Cell = function Cell(xw, xh, xtxt, xborder, xln, xalign, xfill, xlink) {
		...
		if ((this.y) + (xh) > this.PageBreakTrigger && !this.InFooter && this.AcceptPageBreak()) {
			...
			if (xws > 0) {
				this.ws = xws;
				this._out(this.sprintf("%.3f Tw", xws * xk));
			}
		}
		...
		if (xfill == 1 || xborder == 1) {
			...
			xs = this.sprintf("%.2f %.2f %.2f %.2f re %s ", this.x * xk, (this.h - this.y) * xk, xw * xk, -xh * xk, xop);
		}
		if ( typeof xborder === 'string' ) {
			xx = this.x;
			xy = this.y;
			if (lib.is_int(lib.strpos(xborder, "L"))) xs += this.sprintf("%.2f %.2f m %.2f %.2f l S ", xx * xk, (this.h - xy) * xk, xx * xk, (this.h - (xy + xh)) * xk);
			if (lib.is_int(lib.strpos(xborder, "T"))) xs += this.sprintf("%.2f %.2f m %.2f %.2f l S ", xx * xk, (this.h - xy) * xk, (xx + xw) * xk, (this.h - xy) * xk);
			if (lib.is_int(lib.strpos(xborder, "R"))) xs += this.sprintf("%.2f %.2f m %.2f %.2f l S ", (xx + xw) * xk, (this.h - xy) * xk, (xx + xw) * xk, (this.h - (xy + xh)) * xk);
			if (lib.is_int(lib.strpos(xborder, "B"))) xs += this.sprintf("%.2f %.2f m %.2f %.2f l S ", xx * xk, (this.h - (xy + xh)) * xk, (xx + xw) * xk, (this.h - (xy + xh)) * xk);
		}
		if (xtxt != "") {
			if (xalign == "R") {
				xdx = (xw) - (this.cMargin) - (this.GetStringWidth(xtxt))
			} else {
				if (xalign == "C") {
			...
			xtxt = xtxt.replace(/\(/g,'\\(').replace(/\)/g,'\\)').replace(/\\/g,'\\\\');
			if (this.ColorFlag) xs += "q " + this.TextColor + " ";
			xs += this.sprintf("BT %.2f %.2f Td (%s) Tj ET", (this.x + xdx) * xk, (this.h - (this.y + .5 * xh + .3 * this.FontSize)) * xk, xtxt);
	...
	this.MultiCell = function MultiCell(xw, xh, xtxt, xborder, xalign, xfill) {
		...
		if (xborder) {
			...
			{
				xb2 = "";
				if (lib.is_int(lib.strpos(xborder, "L"))) {
					xb2 += "L"
				};
				if (lib.is_int(lib.strpos(xborder, "R"))) {
					xb2 += "R"
				};
			...
			if (xl > xwmax) {
...

				{
					if (xalign == "J") {
						...
						this._out(this.sprintf("%.3f Tw", (this.ws) * this.k));
					}
	...
	this.Image = function Image(xfile, xx, xy, xw, xh, xtype, xlink) {
		...
		this._out(this.sprintf("q %.2f 0 0 %.2f %.2f %.2f cm /I%d Do Q", xw * this.k, xh * this.k, xx * this.k, (this.h - (xy + xh)) * this.k, xinfo["i"]));
	...
	this.Output = function Output( xdest = 'I', xfile = 'doc.pdf', xisUtf8 = false, xres ) {
		if( xres !== undefined ) this.res = xres;
		if (this.state < 3) this.Close();
		if( (xdest.toLowerCase() === 'i' || xdest.toLowerCase() === 'd') && this.res === undefined ) if( this.res === undefined ) {
			console.log('ERROR -- Output cannot send to browser -- no response object provided.');
			return;
		}
		switch( xdest.toLowerCase() ) {
			case 'i':
				this.res.setHeader('ContentType','application/octet-stream');
				this.res.setHeader('Content-disposition', 'attachment; filename=' + xfile);
				this.res.send(this.buffer);  // TODO: if this.hasBinary then output binary..
				break;
			case 'd':
				this.res.setHeader('ContentType','application/pdf');
				this.res.setHeader('Content-Disposition', 'inline');
				this.res.send(this.buffer);  // TODO: if this.hasBinary then output binary..
				break;
			case 'f':
				this.fs.writeFileSync(xfile,this.buffer);
				break;
			case 's':
				return this.buffer;
				break;
			default:
				console.log('ERROR -- Unrecognized output type: "' + xdest + '", options are I (in browser), D (download through browser), F (write to file), or S (return as a string).');
		}
	}
	...
	this._putpages = function _putpages() {
		...
		if (this.AliasNbPages !== '') {
			//for (xn = 1; xn <= xnb; xn++) this.pages[xn] = lib.str_replace(this.AliasNbPages, xnb, this.pages[xn]);  // FIXED
			var rx = new RegExp(this.AliasNbPages.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),'g');
			for (xn = 1; xn <= xnb; xn++) this.pages[xn] = this.pages[xn].replace(rx, xnb); 
		}
		...
		for (xn = 1; xn <= xnb; xn++) {
			...
			//if (lib.isset(this.OrientationChanges[xn])) this._out(this.sprintf("/MediaBox [0 0 %.2f %.2f]", xhPt, xwPt));  // FIXED
			if ( this.OrientationChanges[xn] !== undefined ) this._out(this.sprintf("/MediaBox [0 0 %.2f %.2f]", xhPt, xwPt));
			this._out("/Resources 2 0 R");
			if (this.PageLinks[xn]) {
				xannots = "/Annots [";
				for (xpl in this.PageLinks[xn]) {
					xpl = this.PageLinks[xn][xpl]
					xrect = this.sprintf("%.2f %.2f %.2f %.2f", xpl[0], xpl[1], xpl[0] + xpl[2], xpl[1] - xpl[3]);
			...
			xp = (this.compress) ? this.gzcompress(this.pages[xn]) :
				this.pages[xn];
			this._newobj();
			this._out("<<" + xfilter + "/Length " + xp.length + ">>");
			this._putstream(xp);
			this._out("endobj");
		}
		...
		this._out(xkids + "]");
		this._out("/Count " + xnb);
		this._out(this.sprintf("/MediaBox [0 0 %.2f %.2f]", xwPt, xhPt));
		this._out(">>");
		this._out("endobj");
	}
	...
	this._putinfo = function _putinfo() {
		this._out("/Producer " + this._textstring("FPDF for Node.js v." + this.Version + " [more at https://github.com/Solifugus/fpdf-njs/wiki"));
		if ( this.title    !== '' ) this._out("/Title " + this._textstring(this.title));
		if ( this.subject  !== '' ) this._out("/Subject " + this._textstring(this.subject));
		if ( this.author   !== '' ) this._out("/Author " + this._textstring(this.author));
		if ( this.keywords !== '' ) this._out("/Keywords " + this._textstring(this.keywords));
		if ( this.creator  !== '' ) this._out("/Creator " + this._textstring(this.creator));
		var date = new Date();
		...
		this._out("/CreationDate " + this._textstring("D:" + YYYYMMDDHHMMSS));
	}
	...
	this._enddoc = function _enddoc() {
		...
		this._out(">>");
		this._out("endobj");
		xo = this.buffer.length;
		this._out("xref");
		this._out("0 " + (this.n + 1));
		this._out("0000000000 65535 f ");
		for (xi = 1; xi <= this.n; xi++) this._out(this.sprintf("%010d 00000 n ", this.offsets[xi]));
		this._out("trailer");
		this._out("<<");
		this._puttrailer();
		this._out(">>");
	...
	this._endpage = function _endpage() {
		this.state = 1;
	}
	...
	this._dounderline = function _dounderline(xx, xy, xtxt) {
		...
		return this.sprintf("%.2f %.2f %.2f %.2f re f", xx * this.k, (this.h - (xy - xup / 1000 * this.FontSize)) * this.k, xw * this.k, -xut / 1000 * this.FontSizePt);
	}
	...
	this._escape = function _escape(xs) {
		//return lib.str_replace(")", "\\)", lib.str_replace("(", "\\(", lib.str_replace("\\", "\\\\", xs)));  // FIXED
		return xs.replace(/\(/g,'\\(').replace(/\)/g,'\\)').replace(/\\/g,'\\\\');
	}
